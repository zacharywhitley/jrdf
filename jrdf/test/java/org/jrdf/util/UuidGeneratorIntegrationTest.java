...
public final class UuidGeneratorIntegrationTest extends TestCase {
    ...
    private static final int NUM_UUIDS_TO_GENERATE = UuidGeneratorUnitTest.NUM_UIDS / UuidGeneratorIntegrationTest.NUM_THREADS;
    ...
    public void testMultipleClassLoadersGenerateUniqueUuids() throws Exception {
        URL[] uidClassUrls = new URL[]{ ClassLoader.getSystemClassLoader().getResource(CLASS_NAME) };
        for (int i = 0; i < NUM_CLASSLOADERS; i++) {
            Class<?> uuidGeneratorClass = new URLClassLoader(uidClassUrls).loadClass(FULL_CLASS_NAME);
    ...
    private void generateUuids(Method generateUID) throws Exception {
        Set uuids = new HashSet(UuidGeneratorUnitTest.NUM_UIDS);
    ...
    private static Thread startThread(Set uuids) {
        Thread currentThread = new Thread(new UuidPopulator(uuids, NUM_UUIDS_TO_GENERATE));
        currentThread.start();
        return currentThread;
    }

    private static void waitForThreadCompletion(List threadList) throws InterruptedException {
