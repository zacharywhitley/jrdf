...
final class UuidPopulator implements Runnable {
    ...
    public UuidPopulator(Set uuids, int numUuidsToGenerate){
        this.uuids = uuids;
    ...
    public void run() {
        try {
            ...
            throw new RuntimeException("Error occurred while testing concurrency.", exception);
        }
    }
    ...
    private void checkUuidUnique(String uuid) {
        synchronized (UuidGeneratorIntegrationTest.class) {
            if (uuids.contains(uuid)) Assert.fail("UUID " + uuid + " is not unique");
        }
    }
}
